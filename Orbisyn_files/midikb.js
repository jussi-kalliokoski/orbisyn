...
const onMidiMessage = event => {
  ...
  switch (status) {
    ...
    case 0x80:
      ...
      break;
    default:
      onmidievent(new MidiEvent(channel, status, data1, data2));
  }
};

const selectInput = async (input) => {
  if (selectedInput) {
    selectedInput.onmidimessage = null;
    await selectedInput.close();
    selectedInput = null;
  }
  selectedInput = await input.open();
  selectedInput.onmidimessage = onMidiMessage;
};

const initMidi = async () => {
  midiAccess = await navigator.requestMIDIAccess();
  availableDevs = [...midiAccess.inputs.values()];
  if (availableDevs.length > 0) {
    selectInput(availableDevs[0]);
  }
};
